...
class Graph {
    ...
    size_t dfs_helper(const std::string &start,
        ...
        for (auto &neigh : adj[start]) {
            if (!visited[neigh] || is_uppercase(neigh)) {
                visited[neigh] = true;
                ans += dfs_helper(neigh, visited, small_twice_visit);
                visited[neigh] = false;
            } else if (!small_twice_visit && neigh != "start") {
                ans += dfs_helper(neigh, visited, true);
            }
        }
        return ans;
    }
  ...
  public:
    ...
    size_t get_distinct_paths() {
        ...
        visited["start"] = true;
        return dfs_helper("start", visited, false);
    }
};
